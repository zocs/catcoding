...
| Web OG 封面图升级 | ✅ | `catcoding-web` 新增 `public/og-cover.svg`，替换默认 favicon 作为分享图 |
...
| Web 回归阈值上调 | ✅ | 远端 CI 的 `LH_MIN_PERFORMANCE` 已从 85 提升到 90，并本地复验通过 |
| Web 阈值分层策略 | ✅ | 远端 CI 已改为按分支阈值：`master=95`、其余 `90`，并开启 `LH_SKIP_BUILD=1` |
| Web 工具链稳定化 | ✅ | `lighthouse/http-server` 固化为本地 devDeps，审计脚本改为 `npx --no-install`，减少网络波动 |
...
| Web CI 卡顿防护 | ✅ | Lighthouse 脚本增加进程超时与 `max-wait-for-load`，workflow 步骤增加 `timeout-minutes: 8` |
| Web 性能基线稳态化 | ✅ | 自动化浏览器下禁用 `cat-hunt` 动画注入，修复 CI 中 `/` 性能分数偶发抖动 |
